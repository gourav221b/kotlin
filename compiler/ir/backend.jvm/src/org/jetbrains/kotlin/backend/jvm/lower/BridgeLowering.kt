...
private class BridgeLowering(val context: JvmBackendContext) : FileLoweringPass, IrElementTransformerVoid() {
    ...
    override fun visitClass(declaration: IrClass): IrStatement {
        ...
        val potentialBridgeTargets = declaration.functions.filterTo(mutableListOf(), fun(irFunction: IrSimpleFunction): Boolean {
            ...
            //
            // More subtly, this check is also necessary. There is a corner-case in which we have to produce super calls
            // for special bridge methods. Two of these special bridge methods (MutableMap.remove and getOrDefault)
            // are @PlatformDependent and we cannot create super calls to interface methods on targets before 1.8.
